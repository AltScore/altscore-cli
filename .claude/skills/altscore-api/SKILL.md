...
8. **Ask before you author.** "Create a workflow that does X" leaves business decisions open (who is evaluated, what is decided, where the result lands). Ask them with AskUserQuestion before writing a spec; when that tool is unavailable, stop at `apply --dry-run` and report the open questions. See [workflows-v2](references/workflows-v2.md#discovery-before-authoring-ask-then-build).
