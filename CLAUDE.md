...
- `schema-guide [section] [type]` fetches `/v1/meta/workflows-v2-schema` live from the backend and is the authoritative shape reference. With no argument it prints the server's INDEX (every section with a one-line summary and token cost), so `--help` no longer hardcodes a section list that goes stale. `schema-guide tasks <type>` narrows to one task type; `--full` prints the whole guide (~50k tokens). The CLI itself also fetches `conditionOperators` (`cmd/workflows_v2_normalize.go`) plus `taskTypes`, `workflowCategories`, `relationshipKinds` and `inputSchemaTypes` (`cmd/workflows_v2_apply_vocab.go`). Run the section, do not guess field names from docs.
